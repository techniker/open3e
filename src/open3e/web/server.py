...
def create_app(store: ConfigStore) -> FastAPI:
    ...
    async def _load_depict_from_disk():
        """Read devices.json + Open3Edatapoints_*.py from CWD and upsert DB.

        Returns: (devices_dict, processed_keys) where processed_keys is a set
        of (ecu_address, did) tuples that were upserted. The caller can use
        this set to detect orphans for deletion.
        """
            ...
            if dp_file and os.path.isfile(dp_file):
                try:
                    ...
                    for did, codec in dids.items():
                        did_int = int(did)
                        if codec is None:
                            codec = general_dids.get(did_int)
                        if codec is None:
                            await store.upsert_datapoint(
                                ecu_address=addr, did=did_int,
                                name="DID_" + str(did_int), codec="RawCodec",
                            )
                        else:
                            codec_type_name = type(codec).__name__
                            dp_name = getattr(codec, "id", "DID_" + str(did_int))
                            await store.upsert_datapoint(
                                ecu_address=addr, did=did_int,
                                name=dp_name, codec=codec_type_name,
                            )
                        processed_keys.add((addr, did_int))
                except Exception as e:
                    import logging
                    logging.getLogger(__name__).warning("Error loading %s: %s", dp_file, e)

        return devices, processed_ecus, processed_keys

    async def _restart_engine_from_db():
        """Restart the CAN engine using current DB state."""
        start_engine_fn = getattr(app.state, "start_engine", None)
        if not start_engine_fn:
            return
        try:
            can_iface = await store.get_setting("can_interface")
            can_bitrate_str = await store.get_setting("can_bitrate", "250000")
            ...
            import logging
            logging.getLogger(__name__).warning("Engine restart failed: %s", e)

    @app.post("/api/depict/load")
    async def load_depict_results():
        """Load devices.json and datapoint files generated by system depiction."""
        devices, _, processed_keys = await _load_depict_from_disk()
        await _restart_engine_from_db()
        return {"status": "ok", "devices_loaded": len(devices),
                "datapoints_loaded": len(processed_keys)}

    @app.get("/api/depict/devices")
    async def api_download_devices():
        """Download the current devices.json file (for backup before import)."""
        devices_path = "devices.json"
        if not os.path.isfile(devices_path):
            raise HTTPException(status_code=404, detail="devices.json not found")
        return FileResponse(
    ...
    @app.post("/api/depict/import")
    async def api_import_devices(files: list[UploadFile] = File(...)):
        """Import an existing devices.json + Open3Edatapoints_*.py bundle.

        Steps:
        1. Validate filenames (whitelist).
        2. Validate devices.json parses and references uploaded .py files.
        3. Backup existing devices.json with timestamp suffix.
        4. Stop CAN engine.
        5. Write uploaded files to CWD.
        6. Run _load_depict_from_disk to upsert ECUs/datapoints.
        7. Compute orphans (in DB but not in new config) and delete them.
        8. Publish empty MQTT discovery for orphaned HA entities.
        9. Restart CAN engine.
        Polling preferences (poll_enabled, poll_priority) are preserved by
        the upsert ON CONFLICT clause in upsert_datapoint.
        """
        import logging
        ...
        for f in files:
            ...
            allowed = (
                name == "devices.json"
                or (name.startswith("Open3Edatapoints_") and name.endswith(".py"))
            )
            if not allowed:
                raise HTTPException(
                    status_code=400,
                    detail=f"Disallowed filename: {name}. "
                           "Only devices.json and Open3Edatapoints_*.py accepted.",
                )
            upload_map[name] = await f.read()

        if "devices.json" not in upload_map:
            raise HTTPException(
                status_code=400,
                detail="devices.json must be included in the upload",
            )

        # --- 2. Validate devices.json parses and referenced files exist ---
        try:
            new_devices = json.loads(upload_map["devices.json"].decode("utf-8"))
        except Exception as e:
            raise HTTPException(status_code=400,
                                detail=f"devices.json is not valid JSON: {e}")

        referenced_files: set[str] = set()
        for key, cfg in new_devices.items():
            dp_file = cfg.get("dpList", "")
            if dp_file:
                referenced_files.add(dp_file)

        missing = [
            fn for fn in referenced_files
            if fn not in upload_map and not os.path.isfile(fn)
        ]
        if missing:
            raise HTTPException(
                status_code=400,
                detail=f"devices.json references missing file(s): {', '.join(missing)}. "
                       "Upload them together with devices.json.",
            )

        # --- 3. Backup existing devices.json ---
        backup_name = None
        if os.path.isfile("devices.json"):
            backup_name = f"devices.json.bak.{int(_time.time())}"
            shutil.copy2("devices.json", backup_name)
            log.info("Backed up existing devices.json to %s", backup_name)

        # --- 4. Stop CAN engine ---
        engine = getattr(app.state, "engine", None)
        if engine and engine._thread and engine._thread.is_alive():
            engine.stop()
            log.info("Stopped CAN engine for import")

        # --- 5. Write uploaded files ---
        for name, content in upload_map.items():
            with open(name, "wb") as out:
                out.write(content)
        log.info("Wrote %d uploaded file(s)", len(upload_map))

        # --- 6. Snapshot existing DB state for orphan detection ---
        existing_ecus = await store.get_ecus()
        existing_dps = await store.get_datapoints()
        existing_ecu_addrs = {row["address"] for row in existing_ecus}
        existing_dp_keys = {(row["ecu_address"], row["did"]) for row in existing_dps}

        # --- 7. Run load logic (upserts new state) ---
        try:
            _devices_loaded, processed_ecus, processed_keys = await _load_depict_from_disk()
        except Exception as e:
            # Try to restart engine with old config and bubble error
            await _restart_engine_from_db()
            raise HTTPException(status_code=500, detail=f"Import load failed: {e}")

        # --- 8. Compute and delete orphans ---
        orphan_ecus = existing_ecu_addrs - processed_ecus
        orphan_dps = existing_dp_keys - processed_keys
        # ECU orphans cascade to their DPs, so exclude DPs of orphaned ECUs
        # from the per-DP delete to avoid double-counting
        orphan_dps = {(e, d) for (e, d) in orphan_dps if e not in orphan_ecus}

        # Collect HA entity unique_ids that will be removed (for MQTT cleanup)
        orphan_unique_ids: list[tuple[str, str, int]] = []  # (unique_id, entity_type, ecu_addr)
        if orphan_ecus or orphan_dps:
            all_entities = await store.get_ha_entities()
            ecu_dp_map = {row["id"]: (row["ecu_address"], row["did"])
                          for row in existing_dps}
            for ent in all_entities:
                ent_dict = dict(ent)  # sqlite3.Row -> dict for .get()
                ecu_did = ecu_dp_map.get(ent_dict["dp_id"])
                if ecu_did is None:
                    continue
                ecu, did = ecu_did
                if ecu in orphan_ecus or (ecu, did) in orphan_dps:
                    orphan_unique_ids.append(
                        (ent_dict["unique_id"],
                         ent_dict.get("entity_type") or "sensor",
                         ecu)
                    )

        # Delete orphans — CASCADE handles ha_entities and mqtt_mappings
        # (requires PRAGMA foreign_keys = ON, set in ConfigStore.initialize).
        for addr in orphan_ecus:
            await store._db.execute("DELETE FROM ecus WHERE address = ?", (addr,))
        for ecu, did in orphan_dps:
            await store._db.execute(
                "DELETE FROM datapoints WHERE ecu_address = ? AND did = ?",
                (ecu, did),
            )
        await store._db.commit()
        log.info("Removed %d orphan ECU(s), %d orphan datapoint(s)",
                 len(orphan_ecus), len(orphan_dps))

        # --- 9. Publish empty MQTT discovery for orphaned entities ---
        publisher = getattr(app.state, "mqtt_publisher", None)
        ha_cleared = 0
        if publisher and publisher.connected and orphan_unique_ids:
            ha_prefix = await store.get_setting("ha_discovery_prefix",
                                                "homeassistant") or "homeassistant"
            for uid, etype, ecu in orphan_unique_ids:
                topic = "{}/{}/{}/config".format(ha_prefix, etype, uid)
                try:
                    publisher._client.publish(topic, b"", retain=True)
                    ha_cleared += 1
                except Exception:
                    pass

        # --- 10. Restart CAN engine ---
        await _restart_engine_from_db()
        log.info("Engine restarted after import")

        return {
            "status": "ok",
            "ecus_in_config": len(processed_ecus),
            "ecus_removed": len(orphan_ecus),
            "datapoints_in_config": len(processed_keys),
            "datapoints_removed": len(orphan_dps),
            "ha_entities_cleared": ha_cleared,
